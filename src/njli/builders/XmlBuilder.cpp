...
namespace njli
{
    ...
    s32	XmlBuilder::calculateSerializeBufferSize() const
    {
        return 0;
    }
    ...
    u32 XmlBuilder::getObjectType()const
    {
        return JLI_OBJECT_TYPE_Xml;
    }
    
    const char *XmlBuilder::getClassName()const
    {
        return "XmlBuilder";
    }
    
    s32 XmlBuilder::getType()const
    {
        return XmlBuilder::type();
    }
    ...
    {
        return (XmlBuilder**)World::getInstance()->getWorldFactory()->createArray(XmlBuilder::type(), size);
    }
    ...
    {
        return dynamic_cast<XmlBuilder*>(World::getInstance()->getWorldFactory()->create(XmlBuilder::type()));
    }
    
    XmlBuilder *XmlBuilder::clone(const XmlBuilder &object)
    {
        return dynamic_cast<XmlBuilder*>(World::getInstance()->getWorldFactory()->clone(object, false));
    }
...
//                    printf("%s => %d:%s\n", key, object->getType(), object->getClassName());
    ...
    u32 XmlBuilder::type()
    {
        return JLI_OBJECT_TYPE_XmlBuilder;
    }
}
