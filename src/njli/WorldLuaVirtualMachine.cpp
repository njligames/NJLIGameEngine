    ...
    {  /* repeat for each level */
        ...
        switch (t) {
            ...
            case LUA_TBOOLEAN:  /* booleans */
                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", lua_toboolean(L, i) ? "true\n" : "false\n");
                break;
...
//        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "  ");  /* put a separator */
    ...
    SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "\n");  /* end the listing */
    SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", " --------------- Stack Dump Finished ---------------\n" );
}
...
//    SDL_assert(lua_istable(L,-1)); /* just in case. This is supposed to be module table or parent namespace table */
//    lua_checkstack(L,5);
...
//        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Lua version: %s\n", LUA_VERSION);
        ...
        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "Lua LUA_USE_LONGJMP: yes\n");
#else
        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "Lua LUA_USE_LONGJMP: no\n");
#endif
        
#if defined(__cplusplus)
        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "Lua __cplusplus: yes\n");
#else
        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "%s", "Lua __cplusplus: no\n");
#endif
...
//            int error_code = lua_pcall(m_lua_State, 1, 0, 0);
    ...
    bool WorldLuaVirtualMachine::execute(const char* code, const DeviceTouch &touch)
    {
        if(m_lua_State)
        {
            lua_getglobal(m_lua_State, code);
