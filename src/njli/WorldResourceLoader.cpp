...
//    }
    ...
    bool WorldResourceLoader::FileData::load(const char *filePath)
    {
        ...
        {
            SDL_LogError(SDL_LOG_CATEGORY_TEST, "Unable to open the file: %s", ASSET_PATH(filePath));
        }
        return false;
    }
...
//    }
    ...
    m_type(JLI_IMAGE_TYPE_NONE)
    {
            ...
            (extension == "tga"))
        {
            ...
            SDL_assertPrint(m_buffer, "Failed to load file (%s). Reason: %s", ASSET_PATH(filePath), stbi_failure_reason());
...
//                {
//                    SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Error (%s) : %s", stbi_failure_reason(), buffer);
    ...
    bool WorldResourceLoader::load(const char *filePath, ParticleEmitter *object)
    {
        ...
        if(filePath)
        {
            ...
            if (fileContent)
            {
                ...
                {
                    SDL_LogWarn(SDL_LOG_CATEGORY_TEST, "Unable to parse the ParticleEmitter file");
                }
            }
            else
            {
                if(!fileContent)
                    SDL_LogWarn(SDL_LOG_CATEGORY_TEST, "Particle file content is empty.");
            }
        }
           ...
           fragmentFile)
        {
            ...
            else
            {
                if(!vertexFileContent)
                    SDL_LogWarn(SDL_LOG_CATEGORY_TEST, "Vertex file content is empty.");
                if(!fragmentFileContent)
                    SDL_LogWarn(SDL_LOG_CATEGORY_TEST, "Fragment file content is empty.");
            }
        }
        else
        {
...
//            return true;
        ...
        if (filePath)
        {
            ...
            if(fileContent)
            {
                ...
                {
                    SDL_LogWarn(SDL_LOG_CATEGORY_TEST, "Unable to create the Sound file");
                }
            }
            else
            {
...
//                            bool exists = getFileData(name, &p, &_size);
                            ...
                            {
                                addFileData(name);
                                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Added file: %s - size: %lu\n", name, _size);
                            }
                            else
                            {
                                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "File exists: %s - size: %lu\n", name, _size);
                            }
                        }
                        ++j;
                    }
