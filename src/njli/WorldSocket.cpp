...
namespace njli
{
    ...
    const char *WorldSocket::getClassName()const
    {
        return "WorldSocket";
    }
    
    s32 WorldSocket::getType()const
    {
        return JLI_OBJECT_TYPE_WorldSocket;
    }
    ...
    bool WorldSocket::connectJLI(const char *ip, u16 port)
    {
        ...
        if (res < 0) {
            if (errno == EINPROGRESS) {
                SDL_LogError(SDL_LOG_CATEGORY_TEST, "%s", "EINPROGRESS in connect() - selecting\n");
                do {
                    tv.tv_sec = 1;
                    tv.tv_usec = 0;
                    FD_ZERO(&myset);
                    ...
                    if (res < 0 && errno != EINTR) {
                        ...
                        lon = sizeof(int);
                        if (getsockopt(m_sck, SOL_SOCKET, SO_ERROR, (void*)(&valopt), &lon) < 0) {
                            SDL_LogError(SDL_LOG_CATEGORY_TEST, "Error in getsockopt() %d - %s\n", errno, strerror(errno));
                            disconnectJLI();return false;
                        }
                        // Check the value returned...
                        if (valopt) {
                            SDL_LogError(SDL_LOG_CATEGORY_TEST, "Error in delayed connection() %d - %s\n", valopt, strerror(valopt)
                                    );
                            disconnectJLI();return false;
                        }
                        m_isConnected = true;
                        ...
                        SDL_LogError(SDL_LOG_CATEGORY_TEST, "%s", "Timeout in select() - Cancelling!\n");
                        disconnectJLI();return false;
                    }
                } while (1); 
            } 
    ...
    void WorldSocket::parseMessage(const std::string &delimeter)
    {
        if(isConnected())
        {
                           ...
                           0 )) > 0)
            {
                ...
                if ( (i != -1) && (j != -1) )
                {
                    std::string extract = m_SocketData.substr(i, j + std::string(fullEndDelimeter).length());
                    std::string remainder = m_SocketData.substr(j + std::string(fullEndDelimeter).length(), m_SocketData.length());
...
//            if(len!=-1)
//                SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Received a message of %lld length", len);
            
        }
    }
    
    bool WorldSocket::hasMessage()const
    {
...
//        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Parsed message..\n%s", msg.c_str());
    ...
    void WorldSocket::sendMessage(const std::string &message)
    {
        ...
        if(isConnected())
        {
            ...
            nbytes_sent = send(m_sck,
                               message.c_str(),
                               message.length(),
                               0);
            
//            printf("Bytes_sent: %s -> %zu\n", message.c_str(), nbytes_sent);
        }
