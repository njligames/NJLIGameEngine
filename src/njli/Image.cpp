...
//                return false;
                          ...
                          const btVector2 &sourcePosition)
    {
        ...
        for (u32 x = 0; x < dimension.x(); ++x)
        {
            for(u32 y = 0; y < dimension.y(); ++y)
            {
                ...
                {
                    SDL_LogError(SDL_LOG_CATEGORY_TEST, "error reading (%d, %d)", x, y);
                    error = true;
                }
            }
        }
                         ...
                         const std::string &filename)
    {
        if (dataPtr != NULL && dataSize > 0)
        {
            ...
            SDL_assert(imageType > JLI_IMAGE_TYPE_NONE && imageType < JLI_IMAGE_TYPE_MAX);
            m_njliImageType = imageType;
            
//            SDL_assert(njli::World::getInstance()->getWorldResourceLoader()->dataPtrSize(filename.c_str()) != 0);
            m_Filename = filename;
